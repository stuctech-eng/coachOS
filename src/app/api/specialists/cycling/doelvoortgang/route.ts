...
// ── Doelvoortgang — Cycling Dashboard, Fase 2c ──────────────────────────
...
// Cycling-doel terug, voor weergave op het Dashboard.
export async function GET() {
  try {
    const user = await getUser()
    if (!user) return NextResponse.json({ error: 'Niet ingelogd' }, { status: 401 })

    const goalProgress = await haalGoalsMetProgress(user.id, 'cycling')
    ...
    console.error('[specialists/cycling/doelvoortgang]', err)
