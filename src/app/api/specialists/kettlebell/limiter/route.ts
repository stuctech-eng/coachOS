...
import { bepaalLimiter } from '@/lib/specialists/kettlebell-limiter'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const resultaat = await bepaalLimiter(user.id, discipline, Number(bellWeightKg))
    return NextResponse.json(resultaat)
  } catch (err) {
    console.error('[kettlebell/limiter GET]', err)
