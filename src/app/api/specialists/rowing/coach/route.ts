...
import { analyseerRowing } from '@/lib/specialists/rowing-analysis'
...
export interface RowingCoachAdvies {
  samenvatting: string
...
// ── Rowing Coach Layer — laatste van de 4 openstaande sweep-punten ──────
// Bron: overleg 3 augustus 2026. Exact spiegelbeeld van running/coach/
// route.ts, zelfde "invuloefening"-belofte: genereerCoachPolicy(),
// verwerkKandidaatInzicht(), haalMemoryOp() rechtstreeks hergebruikt,
// geen wijziging nodig. Enige nieuwe stuk: rowing-analysis.ts (bestond
// nog niet, apart gebouwd als eerste stap van deze levering).
//
// Dit is DE plek die Rowing's Coach Memory (route bestond al sinds
// v2.4.232, maar niets vulde 'm automatisch) eindelijk laat werken —
// elke keer dat dit endpoint een kandidaat-inzicht vindt, gaat dat door
// dezelfde Learning Engine als Cycling/Running.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const engineResultaat = await analyseerRowing(user.id, periodDays)

    const policy = await genereerCoachPolicy(user.id)

    let memoryContext = ''
    try {
      const actieveMemory = await haalMemoryOp(user.id, 'rowing', true)
      ...
      console.error('[specialists/rowing/coach] Memory ophalen mislukt, prompt gaat door zonder:', memErr)
    }
    ...
    try {
      ...
      console.error('[specialists/rowing/coach] Morning Health-context ophalen mislukt:', mhErr)
    }

    const [goalProgress, profielRes] = await Promise.all([
      haalGoalsMetProgress(user.id, 'rowing').catch(err => {
        console.error('[specialists/rowing/coach] Goal Engine mislukt, prompt gaat door zonder:', err)
        return []
      }),
      supabase.from('specialist_profiles').select('preferences').eq('user_id', user.id).eq('specialist_type', 'rowing').single(),
    ])
    const voorkeuren = profielRes.data?.preferences || {}
    void voorkeuren // (nog niet gebruikt in de prompt, zelfde als Running — beschikbaar voor een latere uitbreiding)
...
Je bent nu specifiek de Rowing Coach — een specialistische kennislaag
bovenop jezelf, met diepe roeikennis. Je bent GEEN aparte coach met een
andere stem — dezelfde persoonlijkheid als hierboven, alleen met extra
vakkennis over roeien.
...
- Snelheid: ${resultaat.snelheid.gemiddelde_snelheid_m_per_min ? `gemiddeld ${resultaat.snelheid.gemiddelde_snelheid_m_per_min} m/min` : 'geen snelheidsdata'}${resultaat.snelheid.trend_pct !== null ? `, trend ${resultaat.snelheid.trend_pct > 0 ? '+' : ''}${resultaat.snelheid.trend_pct}%` : ''}
- Afstand: ${resultaat.afstand.totaal_km}km totaal${resultaat.afstand.gemiddeld_km_per_activiteit ? `, gemiddeld ${resultaat.afstand.gemiddeld_km_per_activiteit}km per sessie` : ''}
...
Geef een persoonlijk, motiverend maar eerlijk roei-advies. Schrijf in het
Nederlands. Wees concreet — gebruik de cijfers, verzin niets. Respecteer
altijd de Coach Policy hierboven — dit zijn harde grenzen.
...
rest van je antwoord onverhoopt wordt afgekapt:
{
  ...
  "samenvatting": "1-2 zinnen, de kern van waar deze atleet nu staat qua roeien",
  ...
  "kandidaat_inzichten": [
    ...
    let advies: RowingCoachAdvies = {
      samenvatting: 'Nog onvoldoende data voor een roei-samenvatting.',
      sterke_punten: 'Nog geen trend zichtbaar.',
      aandachtspunten: 'Blijf activiteiten importeren voor een vollediger beeld.',
      advies: 'Ga door met roeien en synchroniseer je sessies — over een paar weken kan de coach een beter beeld geven.',
    ...
    try {
      const aiRes = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1200,
          system: systemPrompt,
          messages: [{ role: 'user', content: 'Geef je Rowing Coach-advies.' }],
        }),
      })
        ...
        if (jsonMatch) {
          ...
          for (const kandidaat of kandidaten.slice(0, 2)) {
            ...
            try {
              ...
              console.error('[specialists/rowing/coach] Learning Engine-verwerking mislukt:', leerErr)
            }
          }
        }
      }
    } catch (aiErr) {
      console.error('[specialists/rowing/coach] AI-call mislukt, fallback gebruikt:', aiErr)
    }
    ...
    console.error('[specialists/rowing/coach]', err)
