...
// v2.4.266 (Workout Platform-UI gelijkgetrokken met Rowing): zie
// toelichting in coach/cycling/trainingsplan/page.tsx — zelfde
// gemelde gat, hier voor Running gedicht. pace i.p.v. SPM/watt.
...
interface VertaaldBlok {
  ...
  instruction: string
  looprVertaling: { pace?: string }[]
}

function WorkoutDetail({ sessieId }: { sessieId: string }) {
  const [laden, setLaden] = useState(true)
  const [data, setData] = useState<{ workout: { adaptations: string[]; kruisSportBron?: string }; vertaaldeBlokken: VertaaldBlok[]; uitvoeringsHints: string[]; materiaal: { benodigd: string[]; ontbreekt: string[] }; alternatieven?: { reden: string; workout_id: string }[]; heeftVdot: boolean } | null>(null)
  const [fout, setFout] = useState<string | null>(null)

  useEffect(() => {
    fetch(`/api/specialists/running/training-plan/workout?sessieId=${sessieId}`)
      .then(r => r.json())
  ...
  return (
    <Card className="p-4 mt-1 flex flex-col gap-3">
      ...
      {!data.heeftVdot && (
        <p className="text-xs text-slate-500">💡 Vul een recente wedstrijdprestatie in bij je profiel voor concrete pace-waarden per blok.</p>
      )}
      ...
      <div className="flex flex-col gap-2">
        {data.vertaaldeBlokken.map(blok => {
          const pace = blok.looprVertaling.find(v => v.pace)?.pace
          return (
            <div key={blok.id} className="border-l-2 border-primary-500/50 pl-3">
              ...
              {pace && <p className="text-xs text-primary-400 mt-0.5">{pace}</p>}
            </div>
          )
        })}
      </div>
