...
import { ArrowLeft, Plus, Trash2, Calendar, ChevronDown, ChevronUp } from 'lucide-react'
      ...
      <Card className="p-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              value={endHour ?? ''}
              onChange={e => setEndHour(e.target.value ? Number(e.target.value) : null)}
              className="w-full bg-slate-800 text-white rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary-500"
            >
              <option value="">—</option>
              {Array.from({ length: 24 }, (_, i) => (
        ...
        <div className="grid grid-cols-2 gap-2">
          {RECURRENCE_OPTIONS.map(opt => (
            <button
              ...
              className={`px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${recurrence === opt.value ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-400 active:bg-slate-700'}`}
            >
              {opt.label}
            </button>
          ))}
            ...
            <div className="grid grid-cols-2 gap-2">
              {EVENT_TYPES.map(et => (
                <button key={et.type} onClick={() => selecteerType(et)}
                  className={`flex items-center gap-2 px-3 py-3 rounded-xl text-sm transition-colors text-left ${geselecteerdType?.type === et.type ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-300 active:bg-slate-700'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {RECURRENCE_OPTIONS.map(opt => (
                      <button key={opt.value} onClick={() => setRecurrence(opt.value)}
                        className={`px-3 py-2 rounded-xl text-xs font-medium transition-colors ${recurrence === opt.value ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-400 active:bg-slate-700'}`}>
                        {opt.label}
                      </button>
                    ))}
                  </div>
